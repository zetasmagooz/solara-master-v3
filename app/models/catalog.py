...
class Product(Base):
    ...
    product_type_id: Mapped[int] = mapped_column(Integer, ForeignKey("product_types.id"), default=1)
    brand_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("brands.id"))
    ...
    has_supplies: Mapped[bool] = mapped_column(Boolean, default=False)
    has_modifiers: Mapped[bool] = mapped_column(Boolean, default=False)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
